  ...
  return (
    <div className="sm:h-108 flex sm:flex-row flex-col">
      <BackgroundImageAtom
        ...
        image={allSiteTexts?.paulo?.card_blog?.image}
      >
        <TextAtom
          children
          className="font-bold text-[50px] sm:text-6xl mb-2 text-white sm:w-[700px] leading-[50px] tracking-widest uppercase"
          text={allSiteTexts?.paulo?.card_blog?.title}
        />

        <TextAtom
          children
          className="text-2xl text-white mb-2"
          text={allSiteTexts?.paulo?.card_blog?.subtitle}
        />

        <TextAtom
          children
          className="font-medium text-white text-lg mb-6"
          text={allSiteTexts?.paulo?.card_blog?.message}
        />
      </BackgroundImageAtom>

      <div className="sm:flex hidden flex-row w-auto sm:flex-col justify-between">
        {allSiteTexts?.paulo?.minor_card_blog?.map((element: MinorCardBlogInterface, index: number) => (
          <BackgroundImageAtom
